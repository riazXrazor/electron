...
function BrowserWindowProxy (ipcRenderer, guestId) {
  ...
  const invokeWindowMethod = function (method, ...args) {
    return ipcRenderer.send('ELECTRON_GUEST_WINDOW_MANAGER_WINDOW_METHOD', guestId, method, ...args)
  }

  const invokeWebContentsMethod = function (method, ...args) {
    return ipcRenderer.send('ELECTRON_GUEST_WINDOW_MANAGER_WEB_CONTENTS_METHOD', guestId, method, ...args)
  }

  const invokeWebContentsSyncMethod = function (method, ...args) {
    return ipcRenderer.sendSync('ELECTRON_GUEST_WINDOW_MANAGER_WEB_CONTENTS_METHOD_SYNC', guestId, method, ...args)
  }

  defineProperty(this, 'location', {
    get: function () {
      return invokeWebContentsSyncMethod('getURL')
    },
    set: function (url) {
      url = resolveURL(url)
      return invokeWebContentsSyncMethod('loadURL', url)
    }
  })
  ...
  this.focus = () => {
    invokeWindowMethod('focus')
  }

  this.blur = () => {
    invokeWindowMethod('blur')
  }

  this.print = () => {
    invokeWebContentsMethod('print')
  }
  ...
  this.eval = (...args) => {
    invokeWebContentsMethod('executeJavaScript', ...args)
  }
}
