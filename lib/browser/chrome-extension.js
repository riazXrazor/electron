...
const isChromeExtension = function (contents) {
  const { protocol } = url.parse(contents._getURL())
  return protocol === 'chrome-extension:'
}

ipcMain.on('CHROME_TABS_EXECUTESCRIPT', function (event, requestId, tabId, extensionId, details) {
  if (!isChromeExtension(event.sender)) {
    console.error(`Blocked ${event.sender.getURL()} from calling chrome.tabs.executeScript()`)
    return
  }
